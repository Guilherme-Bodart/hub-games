...
export function CatalogScreen() {
  ...
  const tileWidth = Math.max(150, Math.floor((viewportWidth - 34) / 2));
          ...
          <View style={styles.tilesGrid}>
            {state.readyGames.slice(0, 4).map((game, index) => {
              ...
              return (
                <View
                  key={game.id}
                  className="relative min-h-[220px] overflow-hidden rounded-[18px] border px-2 py-[10px]"
                  style={[styles.gameTile, resolveTileBackgroundStyle(tileTone), { width: tileWidth }]}
                >
                  <LinearGradient
                    pointerEvents="none"
                  ...
                  <View className="flex-1">
                    ...
                    <View className="mt-[6px] flex-row items-center self-center gap-[6px]">
                      <View className="flex-row items-center gap-1 rounded-full border border-white/30 bg-white/20 px-2 py-1">
                        <Text className="font-number text-[10px] leading-[12px] tracking-[0.4px] text-catalog-milk">
